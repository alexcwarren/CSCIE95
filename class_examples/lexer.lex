%{
/* Last revised: 2:44 PM 13-Sep-2016 */
...
/* the following option causes function "yyunput" to not be defined; if this
   option is not present and yyunput is not used, this error:
     warning: yyunput defined but not used [-Wunused-function]
   will be emitted */
%option nounput
/* similar to above for function "input" */
%option noinput
...
#ifdef DEBUG
  printf("malloc_id: Found id \"%s\" with length %ld\n", yytext, yyleng);
  printf("malloc_id: yytext == %p\n", yytext);
#endif
  id = malloc(yyleng+1);
  if(id == (char *)NULL) {
...
#ifdef DEBUG
  printf("malloc_id: returning %p -> \"%s\"\n", id, id);
#endif
  return id;
}
...
#ifdef DEBUG
  printf("eval_num: Found number \"%s\" with length %ld\n", yytext, yyleng);
#endif
...
#ifdef DEBUG
  printf("malloc_string: Found string \"%s\" with length %ld\n", yytext,
	 yyleng);
  printf("malloc_string: yytext == %p\n", yytext);
#endif
  str = malloc(yyleng+1);
  if(str == (char *)NULL) {
    printf("***Out of memory***\n");
...
#ifdef DEBUG
  printf("malloc_str: returning %p -> \"%s\"\n", str, str);
#endif
  return str;
}
